...
async def analyze_and_finalize_interview(
    ...
    for q in questions:
        ...
        if ans and ans.transcript.strip():
            full_transcript += f" {ans.transcript}"
            total_duration += ans.duration_seconds

    is_empty = not bool(full_transcript.strip())
    ...
    if is_empty:
        ...
        feedback = {
            "strengths": ["Started an AI Mock Interview session."],
            "improvements": ["No answers were spoken or typed during this session."],
            "recommendations": ["Speak into your microphone or type your responses to receive a FAANG rubric evaluation."]
        }
    else:
